    ...
    def esegui_scan_strategico(
        ...
        if lista_asset:
            ...
            if isinstance(primo_asset, dict):
                ...
                if "dati_extra" in primo_asset and isinstance(
                    primo_asset["dati_extra"], dict
                ):
                    colonne.extend(primo_asset["dati_extra"].keys())
            else:
                colonne = list(vars(primo_asset).keys())
                if hasattr(primo_asset, "dati_extra") and isinstance(
                    primo_asset.dati_extra, dict
                ):
                    colonne.extend(primo_asset.dati_extra.keys())
        ...
        if lista_asset:
            ...
            extra_sample = (
                sample.get("dati_extra", {})
                if isinstance(sample, dict)
                else getattr(sample, "dati_extra", {})
            )
        ...
        for asset in lista_asset:
            ...
            if rischio_pesato > soglia_critica:
                dettagli_alert.append(
                    f"⚠️ [{config_settore['settore']}] Rischio Critico: {rischio_pesato}."
                )
            ...
            report.append(
                {
                    ...
                    "segnalazioni": (
                        " ".join(dettagli_alert)
                        if dettagli_alert
                        else "Parametri stabili."
                    ),
                }
            )

        logger.info(
            f"Scan {contesto} completato ({config_settore['settore']}). Asset: {len(report)}"
        )
        return report
