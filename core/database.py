...
class DatabaseAziendale:
    def __init__(self):
        ...
        try:
            ...
            logger.info(
                "🛡️ Connessione al Database Supabase RGD-Alpha stabilita con successo!"
            )
    ...
    # --- GESTIONE UTENTI ---
    def crea_utente(
        self, email: str, password_hash: str, ruolo: str = "user", azienda: str = None
    ) -> dict:
        ...
        try:
            ...
            if res.data:
                ...
                if not azienda:
                    ...
                    self.client.table("utenti").update({"azienda": azienda_code}).eq(
                        "id", user_id
                    ).execute()
    ...
    def calcola_e_salva_kpi_correnti(self, user_id: int) -> dict:
        ...
        try:
            ...
            if not res.data or len(res.data) == 0:
                return {
                    "solidita": 100.0,
                    "impatto_30gg": "STABILE",
                    "rischio_medio": 0.0,
                }

            # Estrazione dei valori di rischio
            rischi = [
                row["rischio"] for row in res.data if row.get("rischio") is not None
            ]
            ...
            impatto = (
                "CRITICO"
                if rischio_medio > 7
                else "ATTENZIONE" if rischio_medio > 4 else "STABILE"
            )
    ...
    def supervisione_admin_metriche_globali(self) -> pd.DataFrame:
        ...
        try:
            res = (
                self.client.table("utenti")
                .select("email, ruolo, azienda, data_creazione")
                .execute()
            )
    ...
    def recupera_storia_caricamenti(self, user_id: Any) -> pd.DataFrame:
        ...
        try:
            ...
            res = (
                self.client.table("log_caricamenti")
                .select("*")
                .eq("user_id", clean_id)
