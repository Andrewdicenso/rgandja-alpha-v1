...
class IngestoreDati:
    ...
    def _pulisci_e_riallinea_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(min(len(df), 15)):
            riga_corrente = [
                str(x).lower().strip() for x in df.iloc[i].values if pd.notna(x)
            ]

            punteggio_header = 0
            tutti_i_sinonimi = [
                item for sublist in self.mappa_sinonimi.values() for item in sublist
            ]
    ...
    def _trova_colonna_vera(
        self, df_columns: List[str], categoria: str
    ) -> Optional[str]:
    ...
    def _valida_dati_critici(self, df: pd.DataFrame):
        ...
        if not self._trova_colonna_vera(df.columns, "nome"):
            return (
                False,
                "Colonna Prodotto/Materiale non trovata. Controlla le intestazioni del file.",
            )
    ...
    def _auto_rilevamento_settore(self, colonne):
        ...
        if any(
            t in c_low
            for t in ["fattura", "iban", "lordo", "iva", "invoice", "billing"]
        ):
        ...
        if any(
            t in c_low
            for t in ["cliente", "fornitore", "crm", "vendor", "customer", "partner"]
        ):
    ...
    def elabora_csv(self, file_path, company_id):
        ...
        try:
            ...
            self.db.registra_caricamento(
                company_id, f"Analisi {settore}", os.path.basename(file_path)
            )
